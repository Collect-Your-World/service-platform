...
import (
	...
	userstatus "backend/service-platform/app/internal/auth/constants/user"
...
type AuthManager interface {
	...
	Login(ctx context.Context, req models.AuthUserRequest) (*models.AuthResponse, error)
...
func (d *DefaultAuthManager) Logout(ctx context.Context, req models.LogoutRequest) error {
	claims, err := d.jwtManager.ValidateToken(req.RefreshToken)
...
func (d *DefaultAuthManager) Register(ctx context.Context, req models.RegisterRequest) error {
	_, err := d.repositories.UserRepository.FindByEmail(ctx, req.Email)
	if err == nil {
	...
	_, err = d.repositories.UserRepository.FindByUsername(ctx, req.Email)
	if err == nil {
	...
	hashed, err := d.hasher.HashPassword(req.Password)
	if err != nil {
		return err
	}
...
func (d *DefaultAuthManager) Login(ctx context.Context, req models.AuthUserRequest) (*models.AuthResponse, error) {
	u, err := d.repositories.UserRepository.FindByEmail(ctx, req.Email)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
	...
	// Verify password
	valid, err := d.hasher.CheckPassword(req.Password, u.Password)
	if err != nil {
