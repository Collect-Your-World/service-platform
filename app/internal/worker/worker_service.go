...
import (
	...
	pkgworker "backend/service-platform/app/pkg/worker"
...
func NewWorkerService(res runtime.Resource, workerConfig config.WorkerConfig) *WorkerService {
	...
	jobRepo := jobrepo.NewJobRepository(res)

	// Create Redis queue
	redisQueue := queue.NewRedisQueue(res.Redis.GetUniversalClient(), logger)

	// Create handler registry and register handlers
	handlerRegistry := pkgworker.NewJobHandlerRegistry(logger)
...
func (ws *WorkerService) GetStats() pkgworker.PoolStats {
	return ws.workerPool.GetStats()
}

// GetWorkerPool returns the underlying worker pool
func (ws *WorkerService) GetWorkerPool() pkgworker.Pool {
